...
//         cmd->out_fd = minishell->fd_pipes[1]; 	// assigns minishell->fd_pipes[1] (write end of the pipe) to cmd->out_fd.
//     }
//     else
//     {
...
//  * it means the input should come from a pipe. In this case, it sets cmd->in_fd to minishell->fd_pipes[minishell->nbr_pipes * 2 - 2].
...
//     if (cmd->in_fd == -2)
//         cmd->in_fd = minishell->fd_pipes[minishell->nbr_pipes * 2 - 2];
// }
...
void assign_fds(t_minishell *minishell)
{
    ...
    cmd = minishell->cmds;
    if (cmd->out_fd == -3)
        cmd->out_fd = minishell->fd_pipes[1];
    else
    {
        close(minishell->fd_pipes[1]); 
        minishell->fd_pipes[1] = -2;
    }
    cmd = cmd->next;
    i = 0;
    while (cmd->next)
